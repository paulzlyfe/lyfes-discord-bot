cat > scripts/setup-discord-action.sh <<'BASH'
#!/usr/bin/env bash
set -euo pipefail

BRANCH="discord-notify/centralize-action"
PR_TITLE="chore: centralize discord notify logic into composite action"
PR_BODY="Centralize Discord notify logic into a composite action and entrypoint script.

This change:
- Adds .github/actions/discord-notify/action.yml (composite action)
- Adds .github/actions/discord-notify/entrypoint.sh (executable script)
- Replaces .github/workflows/discord-notify.yml with a minimal workflow that calls the action

Secrets required: DISCORD_WEBHOOK (required), optional DISCORD_BOT_TOKEN and DISCORD_CHANNEL_ID for thread creation. GITHUB_TOKEN is used for check-run lookups when available.

Testing steps:
1. Run the workflow on a test branch and inspect logs.
2. Check the action logs for /tmp/discord_resp when Discord rejects payloads.

Rollback: revert this branch or restore previous workflow file."

# Ensure we're in a git repo
if [ ! -d .git ]; then
  echo "Error: this directory does not look like a git repository. Run this script from your repo root."
  exit 1
fi

# Use existing branch if present, otherwise create it
...
mkdir -p .github/actions/discord-notify

# Write action.yml
cat > .github/actions/discord-notify/action.yml <<'YAML'
...
# Write entrypoint.sh
cat > .github/actions/discord-notify/entrypoint.sh <<'BASH'
#!/usr/bin/env bash
set -euo pipefail
